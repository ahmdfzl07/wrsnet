...
async function loadInvoices() {
  ...
  if (!data?.success) {
    if (tbody)
      tbody.innerHTML =
        '<tr><td colspan="7"><div class="tbl-empty"><p style="color:#dc2626;">Gagal memuat data</p></div></td></tr>';
    return;
  }
  ...
  tbody.innerHTML = data.data
    .map((i) => {
      ...
      const isOverdue =
        i.status === "overdue" ||
        (i.status === "unpaid" && dueObj && dueObj < today2);
      ...
      if (!isPaid && i.due_date) {
        ...
        if (diff < 0)
          dueTxt =
            ...
            _fmtDate(i.due_date) +
            '</span><div style="font-size:10px;color:#dc2626;">' +
            Math.abs(diff) +
            " hari lalu</div>";
        else if (diff <= 3)
          dueTxt =
            ...
            _fmtDate(i.due_date) +
            '</span><div style="font-size:10px;color:#d97706;">' +
            diff +
            " hari lagi</div>";
      }
      ...
      const taxBadge =
        taxAmt > 0
          ? "<div style=\"margin-top:6px;font-size:10.5px;color:#0d9488;font-weight:600;display:inline-flex;align-items:center;gap:6px;background:#f0fdfa;border:1px solid #99f6e4;border-radius:5px;padding:2px 7px;font-family:'Plus Jakarta Sans',-apple-system,sans-serif;letter-spacing:.01em;\">" +
            (taxRatePct > 0
              ? '<span style="background:#0d9488;color:#fff;padding:1px 5px;border-radius:3px;font-size:9.5px;font-weight:700;letter-spacing:.02em;">' +
                taxRatePct +
                "%</span>"
              : "") +
            "Incl. PPN&nbsp;Rp&nbsp;" +
            Number(taxAmt).toLocaleString("id-ID") +
            "</div>"
          : "";
      ...
      const waBtn =
        isOverdue || isUnpaid
          ? `<button class="act-btn act-wa" onclick="sendReminder(${i.id},'${_esc(i.customer?.name || "")}')">
      ...
      if (isPaid && payDate) {
        ...
        stCell +=
          '<div style="font-size:10px;color:#dc2626;margin-top:2px;">Segera bayar</div>';
      }

      return (
        "<tr>" +
        '<td><span style="font-family:monospace;font-size:12px;font-weight:700;color:#0d1b3e;">' +
        _esc(i.invoice_number) +
        "</span></td>" +
        "<td>" +
        '<div style="font-weight:600;color:#0d1b3e;">' +
        _esc(i.customer?.name || "–") +
        "</div>" +
        '<div style="font-size:11px;color:#94a3b8;font-family:monospace;">' +
        _esc(i.customer?.customer_id || "") +
        "</div>" +
        "</td>" +
        '<td style="font-weight:700;color:#1a6ef5;font-size:14px;font-family:monospace;"><div style="display:flex;flex-direction:column;align-items:flex-start;gap:0;">' +
        "<span>" +
        fmtAmt +
        "</span>" +
        taxBadge +
        ...
        dueTxt +
        "</div></td>" +
        '<td class="col-hide-sm" style="font-size:12px;color:#6b7fa8;">' +
        (i.period_month || "–") +
        "/" +
        (i.period_year || "–") +
        "</td>" +
...
window.sendReminder = async function (invoiceId, custName) {
  showBillConfirm(
    ...
    "#16a34a",
    async function () {
      const d = await App.api("/billing/invoices/" + invoiceId + "/reminder", {
        method: "POST",
        body: "{}",
      });
...
window.openGenerate = function () {
  ...
  if (monthEl && !monthEl._previewBound) {
    monthEl.addEventListener("change", loadGeneratePreview);
    monthEl._previewBound = true;
  }
  if (yearEl && !yearEl._previewBound) {
    yearEl.addEventListener("change", loadGeneratePreview);
    yearEl._previewBound = true;
  }
};
...
async function loadGeneratePreview() {
  ...
  try {
    ...
    const eligibleWithPk =
      data.eligible_with_package ?? data.active_with_package ?? 0;
    const eligibleNoPk =
      data.eligible_without_package ?? data.active_without_package ?? 0;
    ...
    if (
      ...
      if (bs.active != null)
        parts.push(
          `<span style="color:#16a34a">active: <b>${bs.active}</b></span>`,
        );
      if (bs.isolated != null)
        parts.push(
          `<span style="color:#dc2626">isolated: <b>${bs.isolated}</b></span>`,
        );
      if (bs.suspended != null)
        parts.push(
          `<span style="color:#d97706">suspended: <b>${bs.suspended}</b></span>`,
        );
      if (bs.inactive != null)
        parts.push(
          `<span style="color:#94a3b8">inactive: <b>${bs.inactive}</b> (di-skip)</span>`,
        );
    ...
    if (data.total_customers === 0) {
      ...
      warnings.push(
        "⚠ Tidak ada customer eligible (active/isolated/suspended) — generate akan 0 invoice.",
      );
    } else if (eligibleWithPk === 0) {
      warnings.push("⚠ Customer eligible belum dipasangkan ke paket apapun.");
    } else if (eligibleNoPk > 0) {
      warnings.push(
        `${eligibleNoPk} customer eligible belum punya paket — akan dilewati.`,
      );
    }
    if (data.existing_in_period > 0) {
      warnings.push(
        `${data.existing_in_period} invoice sudah ada di periode ini — akan dilewati.`,
      );
    }
    warnings.push(
      `<b>Estimasi invoice yang akan dibuat: ${data.estimated_to_generate || 0}</b>`,
    );
...
window.confirmGenerate = async function () {
  ...
  const d = await App.api("/billing/generate", {
    method: "POST",
  ...
  if (d?.success) {
    ...
    if (created === 0 && d.data?.diagnostics) {
      console.warn(
        "[Generate Invoice] Hasil 0 — diagnostic:",
        d.data.diagnostics,
      );
    }
  } else {
...
window.onResetModeChange = function () {
  const mode =
    (document.querySelector('input[name="resetMode"]:checked') || {}).value ||
    "unpaid";
  const periodInputs = document.getElementById("resetPeriodInputs");
  if (periodInputs)
    periodInputs.style.display = mode === "period" ? "grid" : "none";
  loadResetPreview();
};
...
window.onResetConfirmInput = function () {
  ...
  const previewLoaded =
    document.getElementById("resetPreviewContent").style.display !== "none";
...
window.loadResetPreview = async function () {
  ...
  if (mode === "period" && (!month || !year)) {
    if (loadingEl)
      loadingEl.textContent = "Pilih bulan & tahun untuk melihat preview";
    return;
  }
  ...
  try {
    ...
    if (!d?.success) {
      if (loadingEl)
        loadingEl.textContent = d?.message || "Gagal memuat preview";
      return;
    }
    const data = d.data || {};
    const fmt = (n) => "Rp " + Number(n || 0).toLocaleString("id-ID");
    document.getElementById("rpInvoiceCount").textContent = (
      data.invoice_count || 0
    ).toLocaleString("id-ID");
    document.getElementById("rpPaymentCount").textContent = (
...
window.confirmResetInvoices = async function () {
  ...
  try {
    const d = await App.api("/billing/invoices/reset", {
      method: "POST",
...
function _renderPagination(total) {
  ...
  if (_billPage > 1)
    html +=
      '<button class="pg-btn" onclick="_goPage(' +
      (_billPage - 1) +
      ')">← Prev</button>';
  for (
    let i = Math.max(1, _billPage - 2);
    i <= Math.min(totalPages, _billPage + 2);
    i++
  )
    html +=
      '<button class="pg-btn ' +
...
function showBillConfirm(title, body, accentColor, onConfirm) {
  ...
  el.id = "_billConfirm";
  el.style.cssText =
    "position:fixed;inset:0;z-index:99999;background:rgba(13,27,62,.45);backdrop-filter:blur(4px);display:flex;align-items:center;justify-content:center;padding:16px;";
  el.innerHTML =
    '<div style="background:#fff;border-radius:18px;width:100%;max-width:400px;overflow:hidden;box-shadow:0 24px 80px rgba(13,27,62,.25);">' +
    ...
    '<button id="_billOk" style="padding:8px 18px;border:none;border-radius:9px;background:' +
    (accentColor || "#1a6ef5") +
    ';color:#fff;font-weight:700;cursor:pointer;font-size:13px;font-family:inherit;">Ya, Lanjutkan</button>' +
    "</div>" +
    "</div>";

  document.body.appendChild(el);
  document.getElementById("_billCancel").onclick = function () {
    el.remove();
  };
  document.getElementById("_billOk").onclick = function () {
    el.remove();
    onConfirm();
  };
...
function _pmBadge(method) {
  ...
  var cls = ["qris", "gateway"].includes(method)
    ? "pm-" + method
    : ["dana", "ovo", "gopay", "ewallet"].includes(method)
      ? "pm-ewallet"
      : method === "transfer"
        ? "pm-transfer"
        : method === "cash"
...
function _setBar(id, ratio) {
  const el = document.getElementById(id);
  if (el) el.style.width = Math.min(Math.max((ratio || 0) * 100, 2), 100) + "%";
}
function _fmtDate(s) {
  return s
