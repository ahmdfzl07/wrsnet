...
(function (global) {
  ...
  function normalizeConfig(t) {
    ...
    return {
      ...
      showLogo: normalizeBool(
        t.show_logo != null ? t.show_logo : t.showLogo,
        true,
      ),
      logoUrl: t.logo_url || t.logoUrl || "",
      companyName: t.company_name || t.companyName || "",
      companyTagline:
        t.company_tagline || t.companyTagline || "Internet Service Provider",
      ...
      showSubtotal: normalizeBool(
        t.show_subtotal != null ? t.show_subtotal : t.showSubtotal,
        true,
      ),
      showTax: normalizeBool(t.show_tax != null ? t.show_tax : t.showTax, true),
      showDueDate: normalizeBool(
        t.show_due_date != null ? t.show_due_date : t.showDueDate,
        true,
      ),
      showPaymentMethod: normalizeBool(
        t.show_payment_method != null
          ? t.show_payment_method
          : t.showPaymentMethod,
        true,
      ),
      showActiveUntil: normalizeBool(
        t.show_active_until != null ? t.show_active_until : t.showActiveUntil,
        true,
      ),
      showBankInfo: normalizeBool(
        t.show_bank_info != null ? t.show_bank_info : t.showBankInfo,
        false,
      ),
      showSignature: normalizeBool(
        t.show_signature != null ? t.show_signature : t.showSignature,
        true,
      ),
      invoiceLabel: t.invoice_label || t.invoiceLabel || "INVOICE",
      sectionRecipientLabel:
        t.section_recipient_label || t.sectionRecipientLabel || "TAGIHAN UNTUK",
      sectionDetailLabel:
        t.section_detail_label || t.sectionDetailLabel || "DETAIL INVOICE",
      thankYouText:
        t.thank_you_text ||
        t.thankYouText ||
      ...
      footerText:
        t.footer_text ||
        t.footerText ||
        "Dokumen ini di-generate otomatis oleh sistem. Invoice ini sah tanpa tanda tangan basah.",
    };
  }
  ...
  function normalizeData(d) {
    ...
    var rawSubtotal = Number(
      d.invoice_subtotal != null
        ? d.invoice_subtotal
        : d.invoice_amount || d.amount || 0,
    );
  ...
  function renderLogo(cfg, onDark) {
    ...
    if (cfg.logoUrl) {
      return (
        '<div style="width:64px;height:64px;border-radius:14px;display:flex;align-items:center;justify-content:center;background:' +
        (onDark ? "rgba(255,255,255,.15)" : "#fff") +
        ";" +
        (onDark ? "" : "border:1px solid #e2e8f0;") +
        'overflow:hidden;flex-shrink:0;">' +
        '<img src="' +
        escHtml(cfg.logoUrl) +
        '" alt="logo" style="width:100%;height:100%;object-fit:contain;border-radius:14px;" onerror="this.style.display=\'none\';this.parentElement.style.background=\'' +
        bg +
        "';this.parentElement.style.color='#fff';this.parentElement.style.fontSize='30px';this.parentElement.style.fontWeight='800';this.parentElement.textContent='" +
        initial +
        "';\">" +
        "</div>"
      );
    ...
    return (
      '<div style="width:64px;height:64px;border-radius:14px;display:flex;align-items:center;justify-content:center;background:' +
      bg +
      ";color:#fff;font-size:30px;font-weight:800;flex-shrink:0;border:" +
      border +
      ';">' +
      initial +
      "</div>"
  ...
  function renderHeader(cfg, p) {
    if (cfg.headerStyle === "minimal") {
      return (
        "" +
        '<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:30px;padding-bottom:18px;border-bottom:2px solid ' +
        cfg.primary +
        ';">' +
        ...
        cfg.text +
        ';">' +
        escHtml(cfg.companyName) +
        "</div>" +
        '<div style="font-size:11px;color:#94a3b8;">' +
        escHtml(cfg.companyTagline) +
        "</div>" +
        "</div>" +
        "</div>" +
        '<div style="text-align:right;">' +
        '<div style="font-size:28px;font-weight:800;letter-spacing:-.01em;color:' +
        cfg.primary +
        ';">' +
        escHtml(cfg.invoiceLabel) +
        "</div>" +
        "<div style=\"font-family:'DM Mono',monospace;font-size:13px;color:#64748b;margin-top:2px;\">" +
        escHtml(p.invoice_number) +
        "</div>" +
        '<div style="margin-top:8px;display:inline-block;padding:3px 12px;border-radius:20px;background:' +
        lighten(cfg.primary, 40) +
        ";color:" +
        cfg.primary +
        ';font-size:10.5px;font-weight:700;letter-spacing:.08em;">' +
        escHtml(p.statusLabel) +
        "</div>" +
        "</div>" +
        "</div>"
      );
    ...
    if (cfg.headerStyle === "split") {
      ...
      if (cfg.companyAddress)
        ci +=
          "<div>" +
          escHtml(cfg.companyAddress).replace(/\n/g, "<br>") +
          "</div>";
      if (cfg.companyPhone)
        ci += "<div>📞 " + escHtml(cfg.companyPhone) + "</div>";
      if (cfg.companyEmail)
        ci += "<div>✉️ " + escHtml(cfg.companyEmail) + "</div>";
      return (
        "" +
        ...
        '<div style="font-size:32px;font-weight:800;letter-spacing:-.02em;color:' +
        cfg.text +
        ';">' +
        escHtml(cfg.invoiceLabel) +
        "</div>" +
    ...
    return (
      ...
      (cfg.companyPhone
        ? '<div style="font-size:11px;opacity:.75;margin-top:4px;">' +
          escHtml(cfg.companyPhone) +
          "</div>"
        : "") +
      "</div>" +
      "</div>" +
      '<div style="text-align:right;">' +
      '<div style="font-size:10px;font-weight:700;letter-spacing:.15em;opacity:.8;">' +
      escHtml(cfg.invoiceLabel) +
      "</div>" +
      "<div style=\"font-family:'DM Mono',monospace;font-size:18px;font-weight:700;margin-top:3px;\">" +
      escHtml(p.invoice_number) +
      "</div>" +
      '<div style="margin-top:8px;display:inline-block;padding:3px 12px;border-radius:20px;background:rgba(255,255,255,.2);font-size:10px;font-weight:700;letter-spacing:.08em;">' +
      escHtml(p.statusLabel) +
      "</div>" +
      "</div>" +
      "</div>" +
  ...
  function renderRecipient(cfg, p) {
    ...
    return (
      ...
      ";color:" +
      cfg.primary +
      ";padding:2px 8px;border-radius:4px;font-size:10.5px;font-weight:700;font-family:'DM Mono',monospace;margin:4px 0;\">" +
      escHtml(p.cid) +
      "</div>" +
      (p.cust_phone
        ? '<div style="font-size:11px;color:#64748b;margin-top:6px;line-height:1.6;">Kontak: ' +
          escHtml(p.cust_phone) +
          "</div>"
        : "") +
      (p.cust_address
        ? '<div style="font-size:11px;color:#64748b;line-height:1.6;">Alamat: ' +
          escHtml(p.cust_address) +
          "</div>"
        : "") +
      (p.cust_email
        ? '<div style="font-size:11px;color:#64748b;line-height:1.6;">Email: ' +
          escHtml(p.cust_email) +
          "</div>"
        : "") +
      "</div>" +
      '<div style="text-align:right;flex:1;max-width:50%;">' +
      '<div style="font-size:10.5px;font-weight:700;letter-spacing:.12em;text-transform:uppercase;color:#94a3b8;margin-bottom:8px;">' +
      escHtml(cfg.sectionDetailLabel) +
      "</div>" +
      ...
      '<dd style="margin:0;font-weight:700;color:' +
      cfg.text +
  ...
  function renderItems(cfg, p) {
    ...
    return (
      ...
      cfg.text +
      ';">' +
      '<div style="font-weight:700;">Langganan Internet ' +
      escHtml(p.pkg_name) +
      "</div>" +
      '<div style="font-size:10.5px;color:#94a3b8;margin-top:2px;font-weight:400;">Periode: ' +
      escHtml(periodeStr) +
      " · CID: " +
      escHtml(p.cid) +
      "</div>" +
  ...
  function renderTotals(cfg, p) {
    ...
    if (cfg.showSubtotal) {
      rows +=
        '<div style="display:flex;justify-content:space-between;padding:8px 14px;font-size:12px;color:#475569;"><span>Subtotal</span><span>' +
        fmtRp(p.subtotal) +
        "</span></div>";
    }
    if (cfg.showTax && p.tax > 0) {
      rows +=
        '<div style="display:flex;justify-content:space-between;padding:8px 14px;font-size:12px;color:#0d9488;"><span>' +
        escHtml(p.tax_label) +
        (p.taxRate ? " (" + p.taxRate + "%)" : "") +
        "</span><span>" +
        fmtRp(p.tax) +
  ...
  function renderPaymentStatus(cfg, p) {
    ...
    var ref = p.reference_number
      ? '<div><span style="font-size:9.5px;letter-spacing:.1em;text-transform:uppercase;color:#94a3b8;font-weight:700;display:block;margin-bottom:4px;">No. Referensi</span><span style="color:' +
        cfg.text +
        ";font-weight:700;font-family:'DM Mono',monospace;font-size:12px;\">" +
        escHtml(p.reference_number) +
        "</span></div>"
      : "";
    return (
      "" +
      ...
      cfg.text +
      ';font-weight:700;">' +
      escHtml(p.method_label || "–") +
      "</span></div>" +
      ref +
      '<div><span style="font-size:9.5px;letter-spacing:.1em;text-transform:uppercase;color:#94a3b8;font-weight:700;display:block;margin-bottom:4px;">Status</span><span style="color:' +
      statusColor +
  ...
  function renderActiveUntil(cfg, p) {
    ...
    return (
      ...
      '<div><b style="color:#14532d;">Layanan aktif hingga: ' +
      fmtDate(p.due_date) +
      '</b><br><span style="font-size:11px;">Perpanjang sebelum tanggal tersebut agar layanan tidak terputus.</span></div>' +
      "</div>"
    );
  }

  function renderBankInfo(cfg, p) {
    if (!cfg.showBankInfo || !p.bank_accounts || !p.bank_accounts.length)
      return "";
    var rows = p.bank_accounts
      .map(function (b) {
        return (
    ...
    return (
      ...
      (cfg.thankYouText
        ? '<div style="margin-bottom:8px;color:' +
          cfg.text +
          ';font-weight:600;">' +
          escHtml(cfg.thankYouText) +
          "</div>"
        : "") +
      (cfg.footerText
        ? "<div>" + escHtml(cfg.footerText).replace(/\n/g, "<br>") + "</div>"
        : "") +
      "</div>" +
      (cfg.showSignature
  ...
  function buildHtml(cfg, p) {
    var fontStack =
      "'" + cfg.font + "', -apple-system, BlinkMacSystemFont, sans-serif";
    var paperStyle =
      "background:#fff;" +
      "padding:50px 60px;" +
      "font-family:" +
