...
const GeniePage = (() => {
  ...
  const portalCustomer = JSON.parse(
    localStorage.getItem("portal_customer") || "null",
  );

  // ── INIT ────────────────────────────────────────────
  function init() {
    // Load settings dari DB agar persist setelah restart
    fetch("/api/genieacs/settings/load")
      .then((r) => r.json())
      .then((j) => {
  ...
  function renderClientsCell(d) {
    ...
    if (typeof c !== "number" || c < 0)
      return `<span class="dev-empty">—</span>`;
  ...
  async function openDetail(deviceId) {
    ...
    const bwRx = el("bw-rx");
    const bwTx = el("bw-tx");
    if (bwRx) bwRx.textContent = "—";
    if (bwTx) bwTx.textContent = "—";
    ...
    if (cached) {
      ...
      document.getElementById("modal-status-badge").className = cached.online
        ? "status-pill pill-online"
        : "status-pill pill-offline";
      document.getElementById("modal-status-badge").innerHTML =
        `<span class="sdot"></span>${cached.online ? "Online" : "Offline"}`;
    }
    ...
    try {
      ...
      if (j.success && j.data) {
        ...
        el("modal-title").textContent =
          d.model || d.manufacturer || "Detail ONT";
        el("modal-subtitle").textContent =
          d.serial_number || truncate(deviceId, 40);
        ...
        el("d-last-inform").textContent = d.last_inform
          ? fmtDateTime(d.last_inform)
          : "—";

        // WAN & Uptime — tampilkan meski offline (data terakhir)
        el("d-ip").textContent = d.signal?.wan_ip || "—";
        el("d-wan-status").textContent = online
          ? d.signal?.wan_status || "—"
          : "Disconnected";
        el("d-wan-status").style.color = online ? "" : "var(--ot-red)";
        el("d-uptime").textContent = online
          ? d.signal?.uptime_formatted || "—"
          : "—";

        // Sinyal optik & suhu — HANYA tampil saat online
        el("d-rx").textContent =
          online && d.signal?.rx_power ? `${d.signal.rx_power} dBm` : "—";
        el("d-tx").textContent =
          online && d.signal?.tx_power ? `${d.signal.tx_power} dBm` : "—";
        el("d-temp").textContent =
          online && d.signal?.temperature ? `${d.signal.temperature}°C` : "—";
  ...
  async function submitWifi() {
    ...
    if (!ssid && !pass && !ssid5g && !pass5g) {
      showInlineAlert(
        "wifi-result",
        "error",
        "Masukkan SSID atau password yang ingin diubah",
      );
      return;
    }
    ...
    try {
      ...
      if (j.success) {
        showInlineAlert(
          "wifi-result",
          "success",
          "✅ Perintah berhasil dikirim. Perubahan aktif dalam beberapa detik.",
        );
  ...
  function toggle5g() {
    el("wifi-5g-section").style.display = el("wifi-show-5g").checked
      ? "block"
      : "none";
  }
  ...
  function sendTask(action) {
    ...
    const msgs = {
      ...
      "factory-reset":
        "⚠ PERINGATAN! Factory reset akan menghapus SEMUA konfigurasi ONT. Tidak dapat dibatalkan!",
    };
  ...
  async function doConfirm() {
    ...
    try {
      const r = await fetch(
        `/api/genieacs/devices/${safeEncodeId(currentDeviceId)}/${pendingAction}`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
        },
      ...
      showInlineAlert(
        ...
        j.success
          ? `✅ Perintah ${pendingAction} berhasil dikirim`
          : `⚠ ${j.error}`,
      );
    } catch (e) {
  ...
  async function sendRefresh() {
    ...
    try {
      ...
      showInlineAlert(
        ...
        j.success ? "success" : "error",
        j.success
          ? "✅ Refresh dijadwalkan. Data akan diperbarui dalam beberapa detik."
          : "⚠ " + j.error,
      );
  ...
  function resetClients() {
    ...
    if (c)
      c.innerHTML = `<div class="tbl-empty">
      <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><path d="M17 21v-2a4 4 0 00-4-4H5a4 4 0 00-4 4v2"/><circle cx="9" cy="7" r="4"/></svg>
  ...
  async function loadSignalHistory(hours = 6, btn = null) {
    ...
    try {
      ...
      if (sigEl && last) {
        const color =
          last >= -25 ? "#16a34a" : last >= -27 ? "#f59e0b" : "#ef4444";
          ...
          scales: {
            ...
            y: {
              ...
              afterDataLimits: (axis) => {
                axis.max = Math.max(axis.max, -10);
                axis.min = Math.min(axis.min, -35);
              },
            },
          },
        },
      ...
      if (noData) {
        ...
        noData.querySelector("p").textContent = "Error: " + e.message;
      }
    }
  }
  ...
  async function loadBandwidth() {
    ...
    const rxEl = el("bw-rx");
    const txEl = el("bw-tx");
    const rxSub = el("bw-rx-packets");
    const txSub = el("bw-tx-packets");
    const errEl = el("bw-error");
    const srcEl = el("bw-source");
    ...
    try {
      ...
      if (j.customer && srcEl) {
        srcEl.innerHTML =
          `Pelanggan: <strong>${j.customer.name}</strong> (${j.customer.customer_id})` +
          (j.source === "pppoe"
            ? ' &nbsp;·&nbsp; <span style="color:#16a34a">● PPPoE</span>'
            : j.source === "queue"
              ? ' &nbsp;·&nbsp; <span style="color:#1e78ff">● Simple Queue</span>'
              : "");
      }
      ...
      if (!j.success || !j.data) {
        if (rxEl) rxEl.textContent = "—";
        if (txEl) txEl.textContent = "—";
        if (rxSub) rxSub.textContent = "";
        if (txSub) txSub.textContent = "";
        if (errEl) {
  ...
  async function loadAssignedCustomer() {
    ...
    try {
      const r = await fetch(
        `/api/genieacs/devices/${safeEncodeId(currentDeviceId)}/customer`,
      );
      const j = await r.json();
      ...
      if (j.success && j.data) {
        ...
        displayEl.innerHTML =
          '<span style="color:#94a3b8;font-style:italic">Belum ada pelanggan yang di-assign ke ONT ini</span>';
      }
    } catch (e) {
  ...
  async function searchCustomers() {
    ...
    searchTimer2 = setTimeout(async () => {
      ...
      try {
        const r = await fetch(
          `/api/genieacs/customers/search?q=${encodeURIComponent(q)}`,
        );
        const j = await r.json();

        if (!j.success || !j.data?.length) {
          resultsEl.innerHTML =
            '<div style="padding:12px 14px;font-size:13px;color:#94a3b8;text-align:center">Tidak ditemukan</div>';
          resultsEl.style.display = "block";
          return;
        }

        resultsEl.innerHTML = j.data
          .map((c) => {
            const hasOnt = c.ont_sn
              ? `<span style="font-size:10px;color:#f59e0b;margin-left:6px">• Sudah punya ONT</span>`
              : "";
            const statusColor = c.status === "active" ? "#16a34a" : "#ef4444";
  ...
  async function doAssign(customerId, name, custId) {
    ...
    try {
      ...
      const r = await fetch(
        `/api/genieacs/devices/${safeEncodeId(currentDeviceId)}/assign`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
      ...
      if (j.success) {
        showInlineAlert(
          "assign-result",
          "success",
          `✅ ONT berhasil di-assign ke ${name} (${custId})`,
        );
  ...
  function openSettings() {
    ...
    hideInlineAlert("cfg-test-result");
  }
  ...
  async function saveSettings() {
    ...
    const password = el("cfg-password").value;
    if (!nbi_url) {
      showInlineAlert("cfg-test-result", "error", "URL wajib diisi");
      return;
    }
    ...
    try {
      ...
      if (j.success) {
        ...
        el("stat-server-url").textContent = String(nbi_url).replace(
          /^https?:\/\//,
          "",
        );
        loadStats();
  ...
  function hideAlert() {
    el("genie-alert").classList.remove("show");
  }
  ...
  function renderPagination(totalRows, totalPages) {
    ...
    function addBtn(p) {
      if (p === cp) {
        ...
        pages.push(
          `<button class="pg-btn" onclick="GeniePage.gotoPage(${p})">${p}</button>`,
        );
      }
    }
  ...
  function gotoPage(n) {
    ...
    const tableCard =
      document.querySelector(".ot-table-card") ||
      el("genie-tbody").closest(".card");
    if (tableCard) {
      tableCard.scrollIntoView({ behavior: "smooth", block: "start" });
    }
    // Rebuild mobile cards juga
    if (typeof window.rebuildMobileCards === "function")
      window.rebuildMobileCards();
  }
  ...
  function loadClientsByCustomer() {
    if (!devices || !devices.length) return;

    const device = devices.find((d) => d.customer_name === currentCustomerName);

    if (!device) {
      el("clients-summary").textContent = "Device tidak ditemukan";
      return;
    }
    ...
    el("clients-summary").innerHTML =
      `<span style="font-weight:700;color:#0f172a">${device.customer_name}</span>`;

    // ✅ LOAD CLIENTS
    GeniePage.loadClients();
  }
