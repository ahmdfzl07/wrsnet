...
async function loadInfraData(type = currentFilter) {
  ...
  const wantInfra    = true;
  const wantCustomer = !type || type === 'customer';
  const requests = [
    App.api('/infrastructure/map'),
    wantCustomer ? App.api('/customers/map') : Promise.resolve(null),
    App.api('/infrastructure-links')
  ];
  const [allRes, custRes, linksRes] = await Promise.all(requests);
  ...
  // drawDBLinks pakai data yang sudah ter-fetch (sebelumnya dia re-fetch)
...
async function savePoint() {
  ...
  if(tab==='odc'){
    ...
    // Update koordinat customer dijalankan PARALEL dgn save infra, bukan serial.
    // Disimpan di payload._customerCoordPromise untuk di-await bersamaan di bawah.
    payload._customerCoordPromise = App.api(`/customers/${selectedCustomerId}`, {
      method:'PUT', body: JSON.stringify({ latitude: pendingLat, longitude: pendingLng })
    });
  }
  const btn=document.getElementById('saveBtn');
  btn.textContent='Menyimpan...'; btn.disabled=true;
  // Pisahkan customerCoordPromise dari payload sebelum kirim ke API
  const customerCoordPromise = payload._customerCoordPromise;
  if (payload._customerCoordPromise) delete payload._customerCoordPromise;
  try {
    // Jalankan PUT/POST infra + (opsional) PUT customer koordinat secara PARALEL
    const tasks = [
      editId
        ? App.api(`/infrastructure/${editId}`, { method:'PUT', body: JSON.stringify(payload) })
        : App.api('/infrastructure',           { method:'POST', body: JSON.stringify(payload) })
    ];
    if (customerCoordPromise) tasks.push(customerCoordPromise);
...
async function editPoint(id) {
  ...
  if(tab==='odc'){
    ...
    document.getElementById('pop-used').value=pt.used_ports||0;
