...
window.openAddCustomer = async function () {
  ...
  if (idField) {
    ...
    idField.placeholder = "Kosongkan untuk otomatis...";
  }
...
window.closeModal = function () {
  ...
  if (cidInput) {
    ...
    cidInput.style.background = "#eef2f7";
    cidInput.style.cursor = "not-allowed";
  }
  const cidBtn = document.getElementById("ppCidToggle");
  if (cidBtn) {
    cidBtn.textContent = "Ubah";
    cidBtn.style.color = "#1a6ef5";
    cidBtn.style.borderColor = "#e4ecf7";
  }
...
window.toggleIsolate = async function (id, action) {
  ...
  const data = await App.api("/customers/" + id, {
    method: "PUT",
    body: JSON.stringify({
      status: action === "isolate" ? "isolated" : "active",
    }),
  });
  if (data?.success) {
    loadCustomers();
    loadCustomerStats();
    App.showToast("Customer " + label.toLowerCase() + "d", "success");
...
function _confirmCustomerDelete(customer) {
  return new Promise((resolve) => {
    ...
    if (!hasPppoe && !hasRouter)
      syncDisabledReason = "Customer tidak punya PPPoE username & router";
    else if (!hasPppoe)
      syncDisabledReason = "Customer tidak punya PPPoE username";
    else if (!hasRouter)
      syncDisabledReason = "Customer tidak punya router MikroTik terhubung";

    const overlay = document.createElement("div");
    overlay.id = "__custDeleteModal";
    overlay.style.cssText =
      "position:fixed;inset:0;background:rgba(15,23,42,.55);z-index:99999;display:flex;align-items:center;justify-content:center;padding:20px;";

    const box = document.createElement("div");
    box.style.cssText =
      "background:#fff;border-radius:14px;max-width:520px;width:100%;box-shadow:0 20px 60px rgba(0,0,0,.25);overflow:hidden;";
    box.innerHTML = `
    ...
    document.getElementById("__custDelDbOnly").onclick = () => {
      cleanup();
      resolve("db_only");
    };
    document.getElementById("__custDelCancel").onclick = () => {
      cleanup();
      resolve("cancel");
    };
    const escHandler = (e) => {
...
window.deleteCustomer = async function (id, name) {
  ...
  try {
    ...
    if (res?.success && res.data) {
      ...
      if (detail.mikrotik_id && !detail.mikrotik_name) {
        try {
          ...
          console.warn(
            "[deleteCustomer] Gagal fetch device detail:",
            e2.message,
          );
        }
      }
    ...
    console.warn(
      "[deleteCustomer] Gagal fetch detail:",
      e.message,
      "— lanjut dengan info minimal",
    );
  }
  ...
  if (data && data.success) {
    ...
    if (choice === "sync") {
      if (data.router_status === "deleted")
        toastMsg = `Customer & secret di router dihapus (${detail.pppoe_username})`;
...
window.syncDueDates = async function () {
  ...
  showConfirmModal(
    ...
    "Generate invoice untuk semua pelanggan aktif periode <strong>" +
      bulan +
      " " +
      tahun +
      '</strong>?<br><small style="color:#64748b">Invoice yang sudah ada akan dilewati (skip).</small>',
    "calendar",
    "#1a6ef5",
    async function () {
      ...
      if (btn) {
        btn.disabled = true;
        btn.innerHTML =
          '<svg width="14" height="14" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24" style="animation:spin .7s linear infinite"><path stroke-linecap="round" stroke-linejoin="round" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15"/></svg> Memproses...';
      }
      const res = await App.api("/billing/generate", {
        method: "POST",
        body: JSON.stringify({ month: now.getMonth() + 1, year: tahun }),
      });
      // Sinkronisasi due_date invoice dari customer.due_date
      await App.api("/billing/sync-due-dates", { method: "POST" }).catch(
        function () {},
      );
      if (btn) {
        btn.disabled = false;
...
function showConfirmModal(title, body, iconType, accentColor, onConfirm) {
  ...
  el.id = "_confirmModal";
  el.style.cssText =
    "position:fixed;inset:0;z-index:99999;background:rgba(13,27,62,.5);backdrop-filter:blur(4px);display:flex;align-items:center;justify-content:center;padding:16px;animation:fadeIn .15s ease";

  el.innerHTML =
    '<div style="background:#fff;border-radius:20px;width:100%;max-width:420px;overflow:hidden;box-shadow:0 24px 80px rgba(13,27,62,.25);animation:slideUp .2s ease">' +
    '<div style="background:' +
    btnColor +
    ';padding:20px 22px 16px;display:flex;align-items:center;gap:12px;">' +
    ...
    "</div>" +
    "</div>" +
    '<div style="padding:20px 22px;font-size:13.5px;color:#374151;line-height:1.6">' +
    body +
    "</div>" +
    ...
    '<button id="_confirmOk" style="padding:9px 20px;border:none;border-radius:10px;background:' +
    btnColor +
    ";color:#fff;font-weight:700;cursor:pointer;font-size:13px;font-family:inherit;box-shadow:0 3px 10px " +
    btnColor +
    '44">' +
    btnLabel +
    "</button>" +
  ...
  if (!document.getElementById("_confirmStyles")) {
    ...
    s.id = "_confirmStyles";
    s.textContent =
      "@keyframes fadeIn{from{opacity:0}to{opacity:1}}@keyframes slideUp{from{transform:translateY(20px);opacity:0}to{transform:translateY(0);opacity:1}}@keyframes spin{to{transform:rotate(360deg)}}";
    document.head.appendChild(s);
  }

  document.body.appendChild(el);

  document.getElementById("_confirmCancel").onclick = function () {
    el.remove();
  };
...
async function loadCustomerStats() {
  ...
  _setText(
    "scOverduePct",
    overdue > 0
      ? Math.round((overdue / Math.max(active, 1)) * 100) +
  ...
  let revFmt = "Rp 0";
  if (rev >= 1000000)
    revFmt = "Rp " + (rev / 1000000).toFixed(1).replace(".0", "") + "jt";
  ...
  _setBar(
    "scRevenueBar",
    active > 0 ? Math.min(rev / (active * 200000), 1) : 0,
  );
  _setText(
    "scRevenuePct",
    rev > 0
      ...
      : active > 0
        ? "Pelanggan aktif belum punya paket"
  ...
  _setText(
    "scActiveSub",
    Math.round((active / Math.max(total, 1)) * 100) + "% dari total " + total,
  );
  ...
  if (subtitle)
    subtitle.textContent =
      "Manajemen pelanggan, terdapat " + total + " customer terdaftar";
}
...
async function loadCustomers() {
  ...
  if (!data?.success) {
    if (tbody)
      tbody.innerHTML =
        '<tr><td colspan="9" class="empty-state"><p style="color:var(--danger);">Gagal memuat data</p></td></tr>';
    return;
  }
  ...
  if (!data.data?.length) {
    if (tbody)
      tbody.innerHTML =
        '<tr><td colspan="9"><div class="empty-state"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0"/></svg><p>Tidak ada data customer</p></div></td></tr>';
  ...
  tbody.innerHTML = data.data
    .map(function (c) {
      ...
      if (c.latest_due_date) {
        ...
        var fmtDue = due.toLocaleDateString("id-ID", {
          day: "2-digit",
          month: "2-digit",
          year: "numeric",
        });
        if (c.latest_invoice_status === "paid") {
          dueDateHtml =
            fmtDue +
            ' <span style="font-size:10px;background:#f0fdf4;color:#16a34a;padding:1px 6px;border-radius:4px;font-weight:700">Lunas</span>';
        } else if (diffDays < 0) {
          dueDateHtml =
            '<span style="color:#dc2626;font-weight:600">' +
            fmtDue +
            '</span><br><span style="font-size:10px;color:#dc2626">' +
            Math.abs(diffDays) +
            " hari lalu</span>";
        } else if (diffDays === 0) {
          dueDateHtml =
            ...
            fmtDue +
            '</span><br><span style="font-size:10px;color:#ea580c">Hari ini!</span>';
        } else if (diffDays <= 3) {
          dueDateHtml =
            '<span style="color:#d97706;font-weight:600">' +
            fmtDue +
            '</span><br><span style="font-size:10px;color:#d97706">' +
            diffDays +
            " hari lagi</span>";
        } else {
          dueDateHtml = fmtDue;
      ...
      // Status badge
      var dueCk = c.latest_due_date
        ? new Date(c.latest_due_date + "T00:00:00")
        : null;
      ...
      if (c.status === "active")
        isoBtn =
          '<button class="rb rb-iso" onclick="toggleIsolate(' +
          c.id +
          ",'isolate')\">Isolir</button>";
      if (c.status === "isolated")
        isoBtn =
          '<button class="rb rb-act" onclick="toggleIsolate(' +
          c.id +
          ",'activate')\">Aktifkan</button>";

      var addrShort = c.address
        ? _esc(c.address.substring(0, 30)) +
          (c.address.length > 30 ? "..." : "")
        : "";
      var pkgName =
      ...
      return (
        ...
        c.id +
        '" class="cust-name-link">' +
        _esc(c.name) +
        "</a>" +
        (addrShort
          ? '<div style="font-size:11px;color:#6b7fa8;margin-top:1px;max-width:160px;overflow:hidden;white-space:nowrap;text-overflow:ellipsis">' +
            addrShort +
            "</div>"
          : "") +
        "</div>" +
        ...
        (c.pppoe_username
          ? '<div style="font-size:10px;color:#94a3b8;font-family:monospace">' +
            _esc(c.pppoe_username) +
            "</div>"
          : "") +
        (c.static_ip
          ? '<div style="font-size:10px;color:#2563eb;font-family:monospace">IP: ' +
            _esc(c.static_ip) +
            "</div>"
          : "") +
        "</td>" +
        ...
        '<td><span class="sb ' +
        stCls +
        '"><span class="sb-dot" style="background:' +
        stDot +
        '"></span>' +
        stLabel +
        "</span></td>" +
        ...
        '<button class="rb rb-wa" onclick="sendWA(\'' +
        _esc(c.phone || "") +
        "')\" >WA</button>" +
        '<button class="rb rb-invoice" style="background:#f59e0b;color:white" onclick="sendInvoice(' +
        c.id +
        ')">Tagihan</button>' +
        '<button class="rb rb-edit" onclick="editCustomer(' +
        c.id +
        ')">Edit</button>' +
        isoBtn +
        '<button class="rb rb-del" onclick="deleteCustomer(' +
        c.id +
        ",'" +
        _esc(c.name) +
        '\')" title="Hapus">' +
...
function sendWA(phone) {
  if (!phone) {
    App.showToast("Nomor HP tidak tersedia", "error");
    return;
  }
...
function _confirmPppoeRename(oldName, newName) {
  return new Promise((resolve) => {
    ...
    document.getElementById("__ppRenameSync").onclick = () => {
      cleanup();
      resolve("sync");
    };
    document.getElementById("__ppRenameDbOnly").onclick = () => {
      cleanup();
      resolve("db_only");
    };
    document.getElementById("__ppRenameCancel").onclick = () => {
      cleanup();
      resolve("cancel");
    };
    // ESC = cancel
...
async function saveCustomer() {
  if (_custSaving) {
    console.warn(
      "[Customer] saveCustomer dipanggil saat masih in-flight, skip",
    );
    return;
  }
  _custSaving = true;
...
async function _saveCustomerInner() {
  ...
  if (!name) {
    App.showToast("Nama customer wajib diisi", "error");
    return;
  }

  const custId =
    document.getElementById("custId")?.value?.trim().toUpperCase() || "";
  if (!_custEditId && custId) {
    const checkD = await App.api(
      "/customers/check-id?customer_id=" + encodeURIComponent(custId),
    );
    if (!checkD?.available) {
      App.showToast(
        "ID " + custId + " sudah digunakan, pilih ID lain",
        "error",
      );
      return;
    }
  ...
  const createPppoe =
    !_custEditId && !!document.getElementById("custCreatePppoe")?.checked;

  let pppoeData = null;
  if (createPppoe) {
    const mkId =
      document.getElementById("custPppoeRouter")?.value?.trim() || "";
    const pppoeUser = document.getElementById("custPPPoE")?.value?.trim() || "";
    const pppoePass =
      document.getElementById("custPppoePassword")?.value?.trim() || "";
    const pppoeProf =
      document.getElementById("custPppoeProfile")?.value?.trim() || "";
    const pppoeSvc =
      document.getElementById("custPppoeService")?.value || "pppoe";
    const pppoeLocal =
      document.getElementById("custPppoeLocalAddr")?.value?.trim() || "";
    const pppoeRemote =
      document.getElementById("custPppoeRemoteAddr")?.value?.trim() || "";

    if (!mkId) {
      App.showToast("Pilih Router MikroTik untuk PPPoE", "error");
      return;
    }
    if (!pppoeUser) {
      App.showToast("PPPoE Username wajib diisi", "error");
      return;
    }
    if (!pppoePass) {
      App.showToast("Password PPPoE wajib diisi", "error");
      return;
    }
    if (!pppoeProf) {
      App.showToast("Pilih Profile PPPoE", "error");
      return;
    }
  ...
  if (pppoeData) {
    ...
    try {
      const url =
        "/mikrotik/pppoe/secrets?device_id=" +
        encodeURIComponent(pppoeData.device_id);
      ...
      if (!ppRes?.success) {
        ...
        App.showToast(
          "PPPoE gagal: " + errMsg + ". Customer TIDAK disimpan.",
          "error",
        );
        btn.disabled = false;
        btn.textContent = "Simpan Customer";
        return;
      }
      pppoeStatus = "created";
    } catch (e) {
      App.showToast(
        "PPPoE gagal: " + e.message + ". Customer TIDAK disimpan.",
        "error",
      );
      btn.disabled = false;
      btn.textContent = "Simpan Customer";
  ...
  const currentPppoe = (
    document.getElementById("custPPPoE")?.value || ""
  ).trim();
  let pppoeRenameHandled = false; // kalau true → jangan kirim pppoe_username di body PUT

  if (
    _custEditId &&
    _originalPppoeUsername &&
    currentPppoe !== _originalPppoeUsername
  ) {
    // Show modal konfirmasi
    const choice = await _confirmPppoeRename(
      _originalPppoeUsername,
      currentPppoe,
    );
    ...
    btn.textContent =
      choice === "sync" ? "Sync ke router..." : "Update database...";
    try {
      const renameRes = await App.api(
        `/customers/${_custEditId}/rename-pppoe`,
        {
          method: "POST",
          body: JSON.stringify({
      ...
      if (!renameRes?.success) {
        App.showToast(
          "Rename PPPoE gagal: " + (renameRes?.message || "Unknown error"),
          "error",
        );
        btn.disabled = false;
        btn.textContent = "Simpan Customer";
  ...
  if (data?.success) {
    ...
    if (!ppRes.ok) {
      ...
      App.showToast(
        "Customer tersimpan, tapi kredensial portal gagal: " + ppRes.message,
        "warning",
      );
      btn.disabled = false;
      btn.textContent = "Simpan Customer";
    ...
    if (pppoeStatus === "created") {
      App.showToast(
        "PPPoE sudah dibuat di MikroTik tapi customer gagal disimpan: " +
...
function _renderPagination(total, limit) {
  ...
  var info =
    '<div style="font-size:12.5px;color:var(--d-muted)">Menampilkan <strong style=\"color:var(--d-text)\">' +
    (offset + 1) +
    "–" +
    Math.min(offset + limit, total) +
    '</strong> dari <strong style=\"color:var(--d-text)\">' +
    total +
    "</strong> customer</div>";
  var btns = '<div style="display:flex;gap:4px">';
  if (_custPage > 1)
    btns +=
      '<a href="#" class="pg-btn" onclick="_goPage(' +
      (_custPage - 1) +
      ');return false">←</a>';
  for (
    var i = Math.max(1, _custPage - 2);
    i <= Math.min(totalPages, _custPage + 2);
    i++
  ) {
    btns +=
      '<a href="#" class="pg-btn ' +
...
function setIdStatus(type, id) {
  ...
  const map = {
    hint:
      '<span style="color:#adb5bd;font-size:11px;">Berikutnya: <b>' +
      _esc(id) +
      "</b></span>",
    auto:
      '<span style="color:#25d366;font-size:11px;font-weight:600;">✓ Otomatis: ' +
      _esc(id) +
      "</span>",
    available:
      '<span style="color:#25d366;font-size:11px;font-weight:600;">✓ ID tersedia</span>',
    taken:
      '<span style="color:#dc2626;font-size:11px;font-weight:600;">✗ ID sudah digunakan</span>',
    existing:
      '<span style="color:#667781;font-size:11px;">ID tidak bisa diubah</span>',
    checking:
      '<span style="color:#f59e0b;font-size:11px;">⏳ Mengecek...</span>',
  };
...
const _checkIdDebounced = _debounce(async (val) => {
  ...
  const d = await App.api(
    "/customers/check-id?customer_id=" + encodeURIComponent(val),
  );
...
function _clearForm() {
  ...
  if (routerSel)
    routerSel.innerHTML = '<option value="">— Memuat daftar router —</option>';
  const profSel = document.getElementById("custPppoeProfile");
  if (profSel)
    profSel.innerHTML = '<option value="">— Pilih router dulu —</option>';
  ...
  const idStatus = document.getElementById("custIdStatus");
...
async function loadMikrotikDevices() {
  ...
  sel.innerHTML =
    ...
    d.data
      .map(
        (dev) => `<option value="${dev.id}">${dev.name} (${dev.host})</option>`,
      )
      .join("");
}
...
async function loadPppoeRouters() {
  ...
  if (!sel) {
    console.warn("[PPPoE] custPppoeRouter element not found");
    return;
  }
  ...
  try {
    ...
    if (d?.success && Array.isArray(d.data) && d.data.length) {
      ...
      sel.innerHTML =
        ...
        routers
          .map((r) => {
            ...
            const stat =
              r.status === "online" ? "" : " [" + (r.status || "unknown") + "]";
      ...
      if (hint) {
        ...
        hint.innerHTML =
          '<strong style="color:#dc2626">⚠</strong> Belum ada router MikroTik di halaman <a href="/devices" target="_blank" style="color:#2563eb;text-decoration:underline">Devices</a>. Tambahkan router dulu (tipe: Router, aktif).';
      }
    }
  } catch (e) {
...
async function loadPppoeProfiles(deviceId) {
  ...
  if (!sel) {
    console.warn("[PPPoE] custPppoeProfile element not found");
    return;
  }
  ...
  try {
    const url =
      "/mikrotik/pppoe/profiles?device_id=" + encodeURIComponent(deviceId);
    ...
    if (d?.success && Array.isArray(d.data) && d.data.length) {
      ...
      sel.innerHTML =
        '<option value="">Tidak ada profile di router ini</option>';
    } else {
      sel.innerHTML = '<option value="">Gagal muat profile</option>';
      const errMsg =
        d?.message || "Response tidak valid (cek koneksi MikroTik)";
      console.error("[PPPoE] load profiles failed:", errMsg, d);
      const hint = document.getElementById("pppoeFormHint");
      if (hint) {
        hint.style.display = "block";
        hint.innerHTML =
          '<strong style="color:#dc2626">⚠ Gagal muat profile:</strong> ' +
          _esc(errMsg);
      }
    }
  } catch (e) {
    ...
    if (hint) {
      hint.style.display = "block";
      hint.innerHTML =
        '<strong style="color:#dc2626">⚠ Error:</strong> ' + _esc(e.message);
    }
  } finally {
    sel.disabled = false;
    if (spinner) spinner.style.display = "none";
...
(function bindPppoeHandlers() {
  document.addEventListener("change", function (e) {
    ...
    if (t.id === "custPppoeProfile") {
      ...
      if (local)
        parts.push("<strong>Local addr (profile):</strong> " + _esc(local));
      if (remote)
        parts.push("<strong>Pool (profile):</strong> " + _esc(remote));
      if (hint) {
        if (parts.length) {
          hint.style.display = "block";
          hint.innerHTML =
            '<strong style="color:#16a34a">✓ Profile terpilih</strong> &nbsp; ' +
            parts.join(" &nbsp;|&nbsp; ");
        } else {
          hint.style.display = "none";
          hint.innerHTML = "";
...
function ppLoadCredentials(custId) {
  ...
  App.api("/customers/" + custId + "/portal-credentials")
    .then(function (r) {
      if (!r || !r.success || !r.data) {
        if (hint) {
          hint.textContent = "Tidak dapat memuat data kredensial portal";
      ...
      if (hint) {
        if (d.has_custom_password) {
          hint.innerHTML =
            "✓ Password sudah diset. Kosongkan untuk tidak mengubah.";
          hint.style.color = "#16a34a";
        } else if (d.fallback_login_hint) {
          hint.innerHTML =
            'Belum ada password custom — pelanggan login pakai nomor HP: <span style="font-family:DM Mono,monospace;color:#0d1b3e;font-weight:600;">' +
            _esc(d.fallback_login_hint) +
            "</span>";
          hint.style.color = "#b45309";
        } else {
          hint.innerHTML =
            "⚠ Belum ada password & belum ada nomor HP — pelanggan tidak bisa login.";
          hint.style.color = "#dc2626";
        }
      }
      if (last) {
    ...
    .catch(function (e) {
      if (hint) {
        hint.textContent = "Gagal memuat: " + (e.message || "error");
        hint.style.color = "#dc2626";
      }
    });
}
...
window.ppResetPasswordToPhone = async function () {
  ...
  if (
    !confirm(
      "Reset password pelanggan? Setelah reset, pelanggan akan login menggunakan nomor HP-nya sendiri.",
    )
  )
    return;
  try {
    const r = await App.api(
      "/customers/" + _custEditId + "/portal-credentials/reset",
      { method: "POST" },
    );
    if (r && r.success) {
      ...
      App.showToast(
        r && r.message ? r.message : "Gagal reset password",
        "error",
      );
    }
  } catch (err) {
...
async function ppSaveCredentials() {
  ...
  try {
    const r = await App.api(
      "/customers/" + _custEditId + "/portal-credentials",
      {
        method: "POST",
    ...
    if (r && r.success) {
      ...
      if (r.data && r.data.new_password) {
        ppShowResultModal(
          r.data.customer_id || (cidInput && cidInput.value),
          r.data.new_password,
        );
      }
      return { ok: true };
      ...
      if (r && /tidak ada perubahan/i.test(r.message || ""))
        return { ok: true };
      return {
        ok: false,
        message: r && r.message ? r.message : "Gagal update kredensial portal",
      };
    }
  } catch (err) {
...
function ppShowResultModal(custId, password) {
  ...
  const html =
    ...
    '<span id="ppResCid" style="font-size:13.5px;color:#0d1b3e;font-weight:600;">' +
    _esc(custId || "") +
    "</span>" +
    "</div>" +
    ...
    '<span id="ppResPw" style="font-size:13.5px;color:#0d1b3e;font-weight:600;letter-spacing:.05em;">' +
    _esc(password || "") +
    "</span>" +
    "</div>" +
    "</div>" +
...
window.ppCopyResult = function () {
  ...
  const done = function () {
    if (btn) {
      ...
      setTimeout(function () {
        btn.textContent = "Salin Keduanya";
        btn.style.color = "#1a6ef5";
      }, 1500);
    }
  };
