...
function showOk(msg) {
  showToast("toastOk", "✓ " + msg);
}
function showErr(msg) {
  showToast("toastErr", "✕ " + msg);
}
...
function renderStats() {
  ...
  var active = allPackages.filter(function (p) {
    return p.is_active;
  }).length;
  var totalC = allPackages.reduce(function (a, p) {
    return a + (p.customer_count || 0);
  }, 0);
  var activePrices = allPackages
    .filter(function (p) {
      return p.is_active;
    })
    .map(function (p) {
  ...
  var avgPrice = activePrices.length
    ? Math.round(
        activePrices.reduce(function (a, b) {
          return a + b;
...
function renderCards() {
  ...
  if (cat)
    list = list.filter(function (p) {
      return (p.category || detectCat(p)) === cat;
    });
  if (status === "active")
    list = list.filter(function (p) {
      return p.is_active;
...
function buildCard(p) {
  ...
  return (
    ...
    "</div>" +
    '<div class="pkg-desc">' +
    esc(p.description || "Tidak ada deskripsi") +
    "</div>" +
    '<div class="pkg-meta">' +
    ...
    '<button class="rb rb-edit" onclick="openEdit(' +
    p.id +
    ')">' +
    ...
    "</button>" +
    '<button class="rb ' +
    (active ? "rb-toggle-off" : "rb-toggle-on") +
    ...
    "</button>" +
    '<button class="rb rb-del" onclick="openDelete(' +
    p.id +
    ",'" +
    esc(p.name) +
    '\')" title="Hapus">' +
...
function setFilter(f) {
  ...
  if (sel)
    sel.value = f === "active" ? "active" : f === "inactive" ? "inactive" : "";
  ...
  var FILTER_MAP = {
    all: "xc-all",
    active: "xc-active",
    cuscount: "xc-cuscount",
    avgprice: "xc-avgprice",
  };
...
function openEdit(id) {
  var p = allPackages.find(function (x) {
    return x.id === id;
  });
  if (!p) return;
  editingId = id;
...
function clearForm() {
  ["f_name", "f_speed_down", "f_speed_up", "f_price", "f_description"].forEach(
    function (id) {
      var el = document.getElementById(id);
      if (el) el.value = "";
    },
...
async function savePkg() {
  ...
  var speed_down =
    parseInt(document.getElementById("f_speed_down")?.value) || 0;
  ...
  try {
    ...
    if (btnTxt)
      btnTxt.textContent = editingId ? "Simpan Perubahan" : "Simpan Paket";
  }
}
  ...
  if (p && (p.customer_count || 0) > 0) {
    if (msg)
      msg.innerHTML =
        "Paket <strong>" +
        esc(name) +
        "</strong> masih digunakan <strong>" +
        p.customer_count +
        "</strong> pelanggan. Tidak bisa dihapus.";
    if (btn) {
      btn.disabled = true;
      btn.style.opacity = ".5";
    }
    ...
    if (msg)
      msg.innerHTML =
        "Paket <strong>" +
        esc(name) +
        "</strong> akan dihapus permanen. Tindakan ini tidak bisa dibatalkan.";
    if (btn) {
      btn.disabled = false;
      btn.style.opacity = "1";
    }
...
async function confirmDelete() {
  ...
  try {
    var r = await fetch(API + "/packages/" + deletingId, {
      method: "DELETE",
      headers: authH(),
    });
    var j = await r.json();
