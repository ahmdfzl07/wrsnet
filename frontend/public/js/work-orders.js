...
async function loadUsers() {
  try {
    ...
    const opts =
      ...
      allUsers
        .map(
          (u) => `<option value="${u.id}">${esc(u.name || u.email)}</option>`,
        )
        .join("");
  ...
  try {
    ...
    const opts =
      ...
      allCustomers
        .map(
          (c) =>
            `<option value="${c.id}">[${esc(c.customer_id)}] ${esc(c.name)}</option>`,
        )
        .join("");
...
async function loadTickets() {
  try {
    ...
    const opts =
      '<option value="">— Pilih ticket (opsional) —</option>' +
      allTickets
        .map(
          (t) =>
            `<option value="${t.id}">[${esc(t.ticket_number)}] ${esc(t.title)}</option>`,
        )
        .join("");
...
async function loadWOs() {
  try {
    ...
    showErr("Gagal memuat: " + e.message);
  }
}
...
function renderStats() {
  ...
  const pending = allWOs.filter((w) =>
    ["pending", "assigned"].includes(w.status),
  ).length;
  ...
  setText(
    "scTotalPct",
    overdue > 0 ? `${overdue} overdue` : "Tidak ada overdue",
  );
  setText(
    "scPendingPct",
    total ? Math.round((pending / total) * 100) + "% dari total" : "—",
  );
  setText(
    "scProgressPct",
    total ? Math.round((inProgress / total) * 100) + "% dari total" : "—",
  );
  setText(
    "scDonePct",
    total ? Math.round((done / total) * 100) + "% selesai" : "—",
  );
  ...
  setText(
    "woHeaderSub",
    `${total} work order · ${pending} pending · ${inProgress} in progress · ${done} selesai`,
  );
...
function getFiltered() {
  ...
  if (activeStatusFilter)
    list = list.filter((w) => w.status === activeStatusFilter);
...
function renderTable() {
  ...
  tbody.innerHTML = list
    .map((w) => {
      ...
      let techHtml = "";
      if (w.assignedUser) {
        const ini = (w.assignedUser.name || "U")
          .split(/\s+/)
          .map((x) => x[0])
          .slice(0, 2)
          .join("")
      ...
      const custInfo =
        w.is_registration == 1
          ? w.customerRegistration
            ? `<div style="font-size:11px;color:var(--d-muted);margin-top:2px">
              ${esc(w.customerRegistration.name)}
            </div>`
            : ""
          : w.customer
            ? `<div style="font-size:11px;color:var(--d-muted);margin-top:2px">
...
function openDetail(id) {
  ...
  if (w.assignedUser)
    techInfo =
      w.assignedUser.name +
      (w.assignedUser.email ? ` (${w.assignedUser.email})` : "");
  else if (w.technician_name)
    techInfo =
      w.technician_name +
      (w.technician_phone ? ` · ${w.technician_phone}` : "");
    ...
    pending: [
      {
        s: "assigned",
        label: "Tugaskan",
        cls: "background:#1a6ef5;color:#fff;border:none",
      },
      {
        s: "in_progress",
        label: "Mulai",
        cls: "background:#ea580c;color:#fff;border:none",
      },
    ],
    assigned: [
      {
    ...
    in_progress: [
      {
        ...
        cls: "background:#00a07a;color:#fff;border:none",
      },
    ],
    done: [],
    cancelled: [],
...
//     photos = w.photos;
//   } else if (typeof w.photos === "string") {
//     try {
...
//   if (!photos.length) {
//     return '<div style="grid-column:1/-1;text-align:center;padding:16px;color:var(--d-muted);font-size:12.5px">Belum ada foto bukti</div>';
//   }
...
async function updateStatus(id, status) {
  ...
  try {
    const body = { status };
    if (status === "done" && completionNotes)
...
async function uploadPhotos(id, input) {
  ...
  try {
    ...
    showErr("Upload gagal: " + e.message);
  }
  input.value = "";
}
...
async function deletePhoto(id, idx) {
  ...
  try {
    const r = await fetch(`${API}/work-orders/${id}/photos/${idx}`, {
      method: "DELETE",
      headers: authH(),
    });
    const j = await r.json();
...
function fillForm(w) {
  ...
  if (w.assigned_user_id) {
    ...
    setTechType("manual");
    document.getElementById("f_technician_name").value =
      w.technician_name || "";
    document.getElementById("f_technician_phone").value =
      w.technician_phone || "";
  } else {
    setTechType("user");
  }
...
function setTechType(type) {
  techType = type;
  document
    .getElementById("techTypeUser")
    .classList.toggle("active", type === "user");
  document
    .getElementById("techTypeManual")
    .classList.toggle("active", type === "manual");
  document.getElementById("techUserPanel").style.display =
    type === "user" ? "block" : "none";
  document.getElementById("techManualPanel").style.display =
    type === "manual" ? "grid" : "none";
}
...
async function saveWO() {
  ...
  const scheduled_date =
    document.getElementById("f_scheduled_date").value || null;
  const location_address =
    document.getElementById("f_location_address").value.trim() || null;
  ...
  if (techType === "user") {
    assigned_user_id =
      document.getElementById("f_assigned_user_id").value || null;
  } else {
    technician_name =
      document.getElementById("f_technician_name").value.trim() || null;
    technician_phone =
      document.getElementById("f_technician_phone").value.trim() || null;
  }
  ...
  try {
    ...
    let msg = editingId
      ...
      : `Work Order ${j.data?.wo_number || ""} berhasil dibuat`;
    if (j.wa) {
    ...
    btn.disabled = false;
    document.getElementById("btnSaveTxt").textContent = editingId
      ? "Simpan Perubahan"
      : "Buat Work Order";
  }
}
...
async function confirmDelete() {
  ...
  try {
    const r = await fetch(`${API}/work-orders/${deletingId}`, {
      method: "DELETE",
      headers: authH(),
    });
    const j = await r.json();
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  ["woModal", "delModal"].forEach((id) => {
    document.getElementById(id)?.addEventListener("click", (e) => {
      if (e.target === document.getElementById(id)) {
        id === "woModal" ? closeModal() : closeDelModal();
      }
    });
  });
  ...
  // Filters
  let t;
  document.getElementById("woSearch")?.addEventListener("input", () => {
    clearTimeout(t);
