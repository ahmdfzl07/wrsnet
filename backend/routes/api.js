...
router.post('/billing/generate', authenticate, demoGuard, authorize('superadmin', 'admin'), logActivity('generate', 'invoice'), BillingController.generateInvoices);
...
router.post('/app-settings', authenticate, demoGuard, async (req, res) => {
  try {
    ...
    const allowed = [
      ...
      'company_name', 'company_whatsapp', 'snmp_community', 'poll_interval',
      // Portal customer branding
      'portal_hero_image', 'portal_hero_overlay', 'portal_welcome_title', 'portal_welcome_sub',
...
router.post('/app-settings/payment-gateway/test', authenticate, demoGuard, async (req, res) => {
  try {
    ...
    const { AppSetting } = require('../models');
    const rows = await AppSetting.findAll({
      where: { key: ['payment_gateway_enabled','payment_gateway_provider','payment_gateway_env','payment_gateway_server_key','payment_gateway_client_key','payment_gateway_merchant_code'] }
    });
    const cfg = {};
...
// ===== INFRASTRUCTURE =====
router.get('/infrastructure', authenticate, demoGuard, InfrastructureController.index);
router.post('/infrastructure', authenticate, demoGuard, hasPermission('infra_create'), logActivity('create', 'infrastructure'), InfrastructureController.create);
router.get('/infrastructure/map', authenticate, demoGuard, InfrastructureController.mapData);
router.get('/infrastructure/stats', authenticate, demoGuard, InfrastructureController.stats);
...
router.get('/infrastructure/:id', authenticate, demoGuard, InfrastructureController.show);
router.put('/infrastructure/:id', authenticate, demoGuard, hasPermission('infra_update'), logActivity('update', 'infrastructure'), InfrastructureController.update);
router.delete('/infrastructure/:id', authenticate, demoGuard, hasPermission('infra_delete'), logActivity('delete', 'infrastructure'), InfrastructureController.destroy);
...
router.get('/mikrotik/customer-traffic', authenticate, demoGuard, async (req, res) => {
  try {
    ...
    const fetchErrors = {};
    if (queues.status   === 'rejected') fetchErrors.queues   = queues.reason?.message   || String(queues.reason);
    if (sessions.status === 'rejected') fetchErrors.sessions = sessions.reason?.message || String(sessions.reason);
    if (arpRes.status   === 'rejected') fetchErrors.arp      = arpRes.reason?.message   || String(arpRes.reason);
    if (dhcpRes.status  === 'rejected') fetchErrors.dhcp     = dhcpRes.reason?.message  || String(dhcpRes.reason);
    if (Object.keys(fetchErrors).length) {
      const logger = require('../utils/logger');
      logger.warn(`[customer-traffic] partial fetch failure host=${mt.host}: ${JSON.stringify(fetchErrors)}`);
    }

    const queueData   = queues.status   === 'fulfilled' ? (queues.value   || []) : [];
    const sessionData = sessions.status === 'fulfilled' ? (sessions.value || []) : [];
    const arpData     = arpRes.status   === 'fulfilled' ? (Array.isArray(arpRes.value)  ? arpRes.value  : []) : [];
    const dhcpData    = dhcpRes.status  === 'fulfilled' ? (Array.isArray(dhcpRes.value) ? dhcpRes.value : []) : [];
    ...
    queueData.forEach(q => {
      const targets = (q.target || '').split(',').map(t => t.trim().split('/')[0]);
      targets.forEach(ip => { if(ip) queueByTarget[ip] = q; });
    ...
    const result = customers.map(cust => {
      ...
      const queue = (ip        && queueByTarget[ip])
      ...
      const onlineSource = byPPPoE?'pppoe':byARP?'arp':byDHCP?'dhcp':byQueue?'queue':null;
