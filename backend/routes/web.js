...
router.get('/work-orders', authenticate, (req, res) => {
...
router.get('/message-logs', authenticate, (req, res) =>
...
router.get('/invoice/inv/:invoiceId', authenticate, async (req, res) => {
...
router.get('/invoice-template', authenticate, async (req, res) => {
  const { AppSetting } = require('../models');
  const { Op } = require('sequelize');
  const rows = await AppSetting.findAll({ where: { key: { [Op.like]: 'invtpl_%' } } });
  const tplSettings = {};
...
router.get('/invoice-template/preview-print', authenticate, async (req, res) => {
  const tpl = await loadInvoiceTpl();
...
router.get('/voucher-template', authenticate, async (req, res) => {
  const { AppSetting } = require('../models');
  const { Op } = require('sequelize');
...
router.get('/voucher-template/preview-print', authenticate, async (req, res) => {
  const { AppSetting } = require('../models');
  const { Op } = require('sequelize');
  const rows = await AppSetting.findAll({ where: { key: { [Op.like]: 'vtpl_%' } } });
  const tpl = {};
...
router.get('/monitoring/device-monitor', authenticate, (req, res) => {
...
// ── GPS Tracking ───────────────────────────────────────────── 
router.get('/gps-tracking', authenticate, (req, res) =>
  res.render('pages/gps-tracking', { title: 'GPS Tracking', user: req.user, active: 'gps-tracking' }));
 
router.get('/technician-tracking', authenticate, (req, res) =>
