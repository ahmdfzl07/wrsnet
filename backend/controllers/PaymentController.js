...
const METHOD_LABEL = { cash:'Cash', transfer:'Transfer', dana:'DANA', ovo:'OVO', gopay:'GoPay', qris:'QRIS' };
const METHOD_COLOR = { cash:'#059669', transfer:'#2563eb', dana:'#0ea5e9', ovo:'#7c3aed', gopay:'#16a34a', qris:'#d97706' };
...
class PaymentController {
  ...
  async record(req, res) {
    ...
    try {
      const {
        ...
        reference_no, due_date_after, send_wa,
      ...
      if (customer.isolir_status === 'isolated' || customer.isolir_status === 'restoring') {
        try {
          ...
          const restoreResult = await IsolirSvc.restoreAfterPayment(customer.id);
          if (restoreResult?.success && !restoreResult.skipped) {
            console.log(`[Payment] Auto-restore isolir success: customer ${customer.customer_id} (${customer.name})`);
          }
        } catch (e) {
          console.error(`[Payment] Auto-restore isolir gagal untuk customer ${customer.customer_id}:`, e.message);
      ...
      if (send_wa && customer.phone) {
        ...
        try {
          ...
          if (session && WAService.isConnected(session.session_id)) {
            ...
            const methodStr = METHOD_LABEL[payMethod] + (bank ? ` (${bank})` : '');
      ...
      const waSentMsg = waSentStatus === 'sent' ? ' WA konfirmasi terkirim.' : waSentStatus === 'failed' ? ' (Gagal kirim WA)' : '';
      res.status(201).json({
        success: true,
  ...
  async destroy(req, res) {
    ...
    try {
      ...
      await Invoice.update({ status: 'unpaid', paid_date: null }, { where: { id: payment.invoice_id }, transaction: t });
  ...
  async invoiceDataByInvoiceId(req, res) {
    try {
      ...
      if (!rowWithPay) return res.status(404).json({ success: false, message: 'Invoice tidak ditemukan' });
      ...
      const methodMap = { cash:'Tunai (Cash)', transfer:'Transfer Bank', dana:'DANA', ovo:'OVO', gopay:'GoPay', qris:'QRIS' };
      const methodLabel = methodMap[rowWithPay.payment_method] || rowWithPay.payment_method?.toUpperCase() || '-';
  ...
  async invoiceData(req, res) {
    try {
      ...
      const methodMap = { cash:'Tunai (Cash)', transfer:'Transfer Bank', dana:'DANA', ovo:'OVO', gopay:'GoPay', qris:'QRIS' };
      let methodLabel = methodMap[row.payment_method] || row.payment_method?.toUpperCase() || 'Cash';
      if (row.reference_number && row.reference_number.includes(' — ')) {
        const parts = row.reference_number.split(' — ');
        if (parts[0] && !methodMap[parts[0].toLowerCase()]) methodLabel += ' — ' + parts[0];
      }
  ...
  async sendWaInvoice(req, res) {
    try {
      ...
      if (!row) return res.status(404).json({ success: false, message: 'Payment tidak ditemukan' });
      if (!row.cust_phone) return res.status(400).json({ success: false, message: 'Nomor HP pelanggan tidak tersedia' });
      ...
      const periodeStr = (monthNames[row.period_month] || row.period_month) + ' ' + row.period_year;
      const tglBayar  = new Date(row.payment_date + 'T00:00:00').toLocaleDateString('id-ID', { day:'2-digit', month:'long', year:'numeric' });
      const aktifHingga = new Date(row.due_date + 'T00:00:00').toLocaleDateString('id-ID', { day:'2-digit', month:'long', year:'numeric' });
      ...
      await WAService.sendMessage(session.session_id, row.cust_phone, msg, null);
      res.json({ success: true, message: 'Invoice berhasil dikirim via WhatsApp ke ' + row.cust_phone });
    } catch(e) { res.status(500).json({ success: false, message: e.message }); }
  }
