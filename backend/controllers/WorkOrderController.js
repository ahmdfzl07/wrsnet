...
const uploadDir = path.join(
  __dirname,
  "../../frontend/public/uploads/workorders",
);
...
const storage = multer.diskStorage({
  ...
  filename: (req, file, cb) => {
    ...
    cb(
      null,
      `wo_${Date.now()}_${Math.random().toString(36).slice(2, 7)}${ext}`,
    );
  },
});
const upload = multer({
...
const INC = [
  ...
  },
  {
    ...
    attributes: ["id", "name", "phone", "address", "nik"],
    required: false,
  },
  {
    model: Ticket,
...
async function sendWANotification(phone, message) {
  ...
  try {
    ...
    if (!session)
      return { sent: false, reason: "Tidak ada sesi WA yang terkoneksi" };
...
exports.stats = async (req, res) => {
  try {
    ...
    res.json({
      success: true,
      data: { total, pending, inProgress, done, overdue },
    });
  } catch (e) {
    res.status(500).json({ success: false, message: e.message });
  }
...
exports.create = async (req, res) => {
  try {
    ...
    if (!title)
      return res
        .status(400)
        .json({ success: false, message: "Judul WO wajib diisi" });
    ...
    const wo = await WorkOrder.create({
      ...
      technician_phone: technician_phone || null,
    ...
    if (send_wa_notification !== false) {
      ...
      if (techPhone) {
        ...
        waResult = {
          sent: false,
          reason:
            "Nomor HP teknisi tidak tersedia. Isi nomor HP di profil user atau field nomor teknisi.",
        };
      }
    }
...
//     const fields = {};
//     allowed.forEach((k) => {
//       if (k in req.body) fields[k] = req.body[k] || null;
//     });

//     if (fields.status === "in_progress" && !wo.started_at)
//       fields.started_at = new Date();
//     if (fields.status === "done" && !wo.completed_at)
...
exports.uploadPhotos = async (req, res) => {
  try {
    ...
    if (!wo)
      return res
        .status(404)
        .json({ success: false, message: "WO tidak ditemukan" });
    ...
    let photos = normalizePhotos(wo.photos);

    photos = photos.filter((p) => typeof p === "object" && p.url);
    ...
    res.json({
      success: true,
      data: { photos },
      message: `${newPhotos.length} foto berhasil diupload`,
    });
  } catch (e) {
    res.status(500).json({ success: false, message: e.message });
  }
...
//       return res
//         .status(400)
//         .json({ success: false, message: "Index foto tidak valid" });
...
exports.deletePhoto = async (req, res) => {
  try {
    ...
    if (photo?.url) {
      const fp = path.join(__dirname, "../../frontend/public", photo.url);
