...
class CustomerController {
  async getOdp(req, res) {
    try {
      const { InfrastructurePoint } = require("../models");

      const data = await InfrastructurePoint.findAll({
        where: { type: "odp" },
        attributes: ["id", "name"],
        order: [["name", "ASC"]],
      });
  ...
  async index(req, res) {
    try {
      ...
      if (odp_id) {
        const infraCustomers = await InfrastructurePoint.findAll({
          where: {
            type: "customer",
          },
          raw: true,
        ...
        const filtered = infraCustomers.filter((i) => {
          return i.parent_id == odp_id;
        });
        ...
        where.id = {
          [Op.in]: customerIds.length ? customerIds : [0],
        };
      }
