...
const apiLimiter = rateLimit({
  ...
  skip: (req) => {
    const path = req.path || "";
    return /^\/(notifications\/unread-count|dashboard\/stats|auth\/profile)/.test(
      path,
    );
  },
...
app.use(
  ["/api/auth/login", "/api/auth/register", "/portal/api/login"],
  authLimiter,
);
...
app.use(async (req, res, next) => {
  try {
    ...
    if (
      _appSettingsCache &&
      now - _appSettingsCacheAt < APP_SETTINGS_CACHE_TTL
    ) {
